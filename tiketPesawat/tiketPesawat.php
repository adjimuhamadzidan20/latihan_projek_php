<?php  
	...
	// paginasi
	$jumlahDataPerHal = 10;
	$isiData = count(show_data("SELECT * FROM data_pemesan"));
	$jumlahHal =  ceil($isiData / $jumlahDataPerHal) + 1;
	...
	// hal 1 = 0 sd 9
	// hal 2 = 10 sd 16
	$dataAwal = ($jumlahDataPerHal * $posisiHal) - $jumlahDataPerHal;
	$result = show_data("SELECT * FROM data_pemesan LIMIT $dataAwal, $jumlahDataPerHal");

	// mencari data
	if (isset($_POST['cari'])) {
		$sql = "SELECT * FROM data_pemesan WHERE Nama_pemesan LIKE '%$_POST[inputCari]%' LIMIT $dataAwal, $jumlahDataPerHal";
...
<head>
	...
	<link rel="stylesheet" href="asset/css/tiketPesawat.css">
		...
		<div class="tabel-data">
			...
			<div class="paginasi">
				<?php for ($i = 1; $i < $jumlahHal; $i++) : ?>
					<?php if ($i == $posisiHal) : ?>
						<a href="?page=<?= $i; ?>" style="background-color: #273c75; padding: 4px 6px; color: white; text-decoration: none; font-size: 13px;">
							<?= $i; ?>
						</a>
					<?php else : ?>
						<a href="?page=<?= $i; ?>" style="background-color: lightgrey; padding: 4px 6px; text-decoration: none; font-size: 13px; color: grey;">
							<?= $i; ?>
						</a>
					<?php endif; ?>
				<?php endfor; ?>
